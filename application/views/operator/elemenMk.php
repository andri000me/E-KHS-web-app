...
<script>
	$(document).ready(function () {
		...
		// ==========get data===============
		var url = "<?php echo site_url('operator/elemen_mk/get_data')?>";
		...
		$('.add-data').click(function (e) {
			e.preventDefault();
			var url = "<?php echo base_url('operator/elemen_mk/add')?>";
			var data = {
		...
		$('tbody').on('click', '.edit', function () {
			...
			let url_1 = "<?php echo base_url('operator/elemen_mk/get_elemenById')?>";
			var dt_set = function (data) {
				$('#my-modal [name="kode"]').val(data.elemenmk);
		...
		$('.edit-data').click(function (e) {
			var url = "<?php echo base_url('operator/elemen_mk/update')?>";
			e.preventDefault();
			var data = {
		...
		$('tbody').on('click', '.hapus', function () {
			var url = "<?php echo base_url('operator/elemen_mk/delete')?>";
			let data = table.row($(this).parents('tr')).data();
			id = data[0];
			hapus(url, id);
			table.ajax.reload();
